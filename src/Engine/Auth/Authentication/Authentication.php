    ...
    public function authinticate(Credentials $credentials) : StructuredResponse {
        ...
        if ($account = $this->identification->identify($credentials->userName)){
            ...
            if (password_verify($credentials->userPassword, $account->passwordHash)){
                 ...
                 * Если украдут токены, пользователь вынужден будет заного войти в систему по окончанию своего access token'a,
                 ...
                 * Не думаю что это нужно, так как данное ДТО более удобно для работы внутри бизнеслогики домена/модели
                 * JsonResponse имеет смысл использовать в Middleware и Controllers
                 */
                return new StructuredResponse(200, 'Пользователь аутентифицирован');//
            }
